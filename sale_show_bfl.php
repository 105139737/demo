...
$('#data8').load('sale_list_bfl.php?fdt='+fdt+'&tdt='+tdt+'&snm='+encodeURIComponent(snm)+'&brncd='+brncd+'&prnm='+prnm+'&tp1='+tp1+'&gstin='+gstin+'&godown='+godown+'&stat='+stat+'&scat='+scat+'&cat='+cat+'&sale_per='+sale_per+'&delv='+delv+'&refsl='+refsl+'&ddt='+ddt).fadeIn('fast');
}
function view(blno)
{
var ddt= document.getElementById('ddt').value;
...
{
$('#can99').load('bfl_sale_invc.php?billno='+encodeURIComponent(billno)).fadeIn('fast');
}

}
function bfl_bst(billno)
{
	
if(confirm("Are you sure  ??"))
{
$('#can99').load('bfl_bst.php?billno='+encodeURIComponent(billno)).fadeIn('fast');
}
...
function upload_bfl(blno)
{
	$('#cnt11').load('bfl_upload.php?blno='+blno).fadeIn("fast");
...
$customer="  and FIND_IN_SET(sl, '$cust')>0 ";
}
?>
