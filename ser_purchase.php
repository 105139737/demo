...
function add()
{
	...
	{
	 $('#wb_Text13').load("ser_adtmppr.php?prnm="+prnm+"&unit="+unit+"&usl="+usl+"&qnty="+qnty+"&mrp="+mrp+"&total="+total+"&disp="+disp+"&disa="+disa+"&ldis="+ldis+"&ldisa="+ldisa+'&lttl='+lttl+'&fst='+fst+'&tst='+tst+'&cgst_rt='+cgst_rt+'&sgst_rt='+sgst_rt+'&igst_rt='+igst_rt+'&cgst_am='+cgst_am+'&sgst_am='+sgst_am+'&igst_am='+igst_am+'&net_amm='+net_amm+'&bcd='+bcd+'&rate='+rate+'&betno='+betno+'&tsl='+tsl).fadeIn('fast');
	}
	}
	function reset()
	{
	...
	$('#wb_Text13').load("ser_tmppr.php").fadeIn('fast');
	...
	function deltpr1(sl)
	{
	$('#wb_Text13').load("ser_deltpr.php?sl="+sl).fadeIn('fast');
	}
...
<form method="post" name="form1" id="form1" action="ser_purchases.php" enctype="multipart/form-data">
...
<td>
<input type="text" class="form-control"  id="dt" tabindex="7"  name="dt" value="<?php  echo date('d-m-Y');?>" width="100%" placeholder="Enter Date">
</td>
</tr>
<tr>
...
<td align="right"   width="30%"><b>File Upload :</b></td>
<td colspan="3">
...
<input type="hidden" name="tamm1" id="tamm1" tabindex="37"  class="form-control" style="background-color:#f3f4f5;" onkeypress="return isNumber1(event)"> 
 ...
 <td colspan="2">
 <b>Remark's :</b>
 <input type="text" class="form-control" id="remk" tabindex="38"  name="remk" value="" size="25" >
 </td>
 <td>
 <b>Add/Less</b><br>
 <table>
...
<input type="submit" class="btn btn-success" id="Button2" tabindex="48"  onclick="return confirm('Are Yoy Sure To Submit !'); " name="bt1" tabindex="15" value="Submit"  >
</td>
</tr>
</table>
