...
$file="Sales_Person_Target_Report_as_".$fdt."_To_".$tdt.".xls";
...
<span style="float:left"><h5>SECONDARY TGT VS ACH-MTD (<?php  echo date('d-M-Y', strtotime($fdt))?> To <?php  echo date('d-M-Y', strtotime($tdt))?>)</h5></span><br>
...
while ($row15 = mysqli_fetch_array($data15))
{
	...
	$ach=0;
		$netamm1=0;
		$data18= mysqli_query($conn,"SELECT sum(pcs) as ach, sum(net_am) as netamm  FROM main_billdtls where sl>0 and scat='$catsl' and find_in_set(blno,'$blno1')>0  and can='0'") or die(mysqli_error($conn));
...
<div>
<span style="float:left"><h5>COLLECTION TGT VS ACH-MTD</h5></span><br>
<table border="1" class="table table-bordered" style="width:60%;" align="left">
<tr>
