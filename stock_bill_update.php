...
<?php 
$l=0;
$data1= mysqli_query($conn,"select * from  main_billing where sl>0")or die(mysqli_error($conn));
...
$expdt=$row['expdt'];
$bccnt++;
 $query8=mysqli_query($conn,"Select * from ".$DBprefix."stock where pcd='$pcd' and stout='$qty' and betno='$betno' and expdt='$expdt' and dt='$edt' and nrtn='Sale' and dtm='$pdts' and refno='' and bcd='$bcd'")or die(mysqli_error($conn));
 
  while ($R5 = mysqli_fetch_array ($query8))
{
$stout=$R5['stout'];
$sl=$R5['sl'];
$up="update ".$DBprefix."stock set sbill='$blno' where sl='$sl'";
