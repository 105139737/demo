...
<?php 
$l=0;
$data1= mysqli_query($conn,"select * from  main_purchase where sl>0")or die(mysqli_error($conn));
...
$expdt=$row['expdt'];
$bccnt++;
 $query8=mysqli_query($conn,"Select * from ".$DBprefix."stock where pcd='$pcd' and stin='$qty' and betno='$betno' and expdt='$expdt'  and nrtn='Purchase' and dtm='$pdts' and refno=''")or die(mysqli_error($conn));
 
  while ($R5 = mysqli_fetch_array ($query8))
{
$stout=$R5['stin'];
$sl=$R5['sl'];
$up="update ".$DBprefix."stock set pbill='$blno' where sl='$sl'";
...
<?php 
$l1=0;
$data2= mysqli_query($conn,"select * from  main_purchase where sl>0")or die(mysqli_error($conn));
