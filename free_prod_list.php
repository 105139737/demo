...
<th style="text-align:left">Product Name</font></th>
 </tr>

<?php 
$sl=$start;
$sln=0;
$datatt= mysqli_query($conn,"select * from main_free_product where sl>0")or die(mysqli_error($conn));
$rcntttl=mysqli_num_rows($datatt);
$datar= mysqli_query($conn,"select * from main_free_product where  sl>0".$all1.$cat1.$bnm1)or die(mysqli_error($conn));
$rcnt=mysqli_num_rows($datar);
$data= mysqli_query($conn,"select * from main_free_product where  sl>0 $all1 $cat1 $bnm1 order by pnm limit $start,$ps ")or die(mysqli_error($conn));
