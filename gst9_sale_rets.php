...
$file="sale_Return_".date('Ymdhis').".xls";
...
$data= mysqli_query($conn,"select cgst_rt,sgst_rt,igst_rt,sum(tamm) as amm,sum(cgst_am) as cgst_am,sum(sgst_am) as sgst_am,sum(igst_am) as igst_am from main_billdtls_ret where sl>0 $todt group by (sgst_rt+cgst_rt+igst_rt) order by cgst_rt,igst_rt")or die(mysqli_error($conn));
