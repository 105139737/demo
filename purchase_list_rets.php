...
{
$data1= mysqli_query($conn,"select * from main_purchase_ret where sl>0".$todt.$snm1.$brncd1." order by dt,sl")or die(mysqli_error($conn));
}
else
{
$data1= mysqli_query($conn,"select * from main_purchase_ret where sl>0 ".$todt.$snm1.$brncd1." order by dt,sl")or die(mysqli_error($conn));
...
$tigst_am=0;
$tgst=0;
$data= mysqli_query($conn,"select * from  main_purchasedet_ret where sl>0 and blno='$blno' $catsl1 $scatsl1 $prnm1 $godown1")or die(mysqli_error($conn));
...
if($blno1==$blno)	
{
	...
	<td  align="left" ><b><?php  echo $spn;?></b></td>
	<?php 
	}
	else
	{
...
$dis11=$dis11+$dis1;
$dis111=(float)$dis111+(float)$ldisa1;
	$amm2=$amm1+$amm2;
	$wgamm=$wgamm+$roff;
	if($tamm2>0)
	{
