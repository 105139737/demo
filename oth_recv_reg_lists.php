...
{
$file="Collection Report Details (".$fdt." To ".$tdt.".xls";
header("Content-type: application/vnd.ms-excel"); 
header("Content-Disposition: attachment; filename=$file"); 
}


$datattg= mysqli_query($conn,"SELECT sum(amm) as gtotal FROM main_drcr where cldgr='4' and retn_stat!='1'".$pnoo.$brncd1.$custid1.$todt)or die(mysqli_error($conn));
...
<thead>
<tr>
<td align="right" colspan="7"><font size="3"> <b> Grand Total : </b></font></td>
<td align="center"><font size="3"> <b> <?php echo number_format($gtotal??0,2);?> </b></font></td>
<td align="center"></td>
<td align="center"></td>
<td align="center"></td>
<td align="center"></td>
...
<tr>
<td align="right" colspan="7"><font size="3"> <b> Total : </b></font></td>
...
$total_am=0;
$datatt= mysqli_query($conn,"SELECT * FROM main_drcr where cldgr='4' and retn_stat!='1' and typ='77'".$pnoo.$brncd1.$custid1.$todt)or die(mysqli_error($conn));
$rcntttl=mysqli_num_rows($datatt);
$datar= mysqli_query($conn,"SELECT * FROM main_drcr where cldgr='4' and retn_stat!='1' and typ='77'".$pnoo.$brncd1.$custid1.$todt)or die(mysqli_error($conn));
$rcnt=mysqli_num_rows($datar);
if($val=="0")
{
$data= mysqli_query($conn,"SELECT * FROM main_drcr where cldgr='4' and retn_stat!='1' and typ='77' $pnoo $brncd1 $custid1 $todt $slp1 order by dt Desc,sl desc limit $start,$ps")or die(mysqli_error($conn));
}
else
{
$data= mysqli_query($conn,"SELECT * FROM main_drcr where cldgr='4' and retn_stat!='1' and typ='77' $brncd1 $custid1 $todt $slp1 order by dt Desc,sl desc")or die(mysqli_error($conn));
}
	$sln++;
		...
		$sl++;
						$data1= mysqli_query($conn,"SELECT * FROM main_ledg where sl='$cldgr'");
							...
							$SSM= mysqli_query($conn,"select * from main_billing where blno='$cbill'")or die(mysqli_error($conn));
							while ($SMS = mysqli_fetch_array($SSM))
							{
							$invto=$SMS['invto'];
							$sfno=$SMS['sfno'];
							}
								$SSM1= mysqli_query($conn,"select * from  main_cust where sl='$invto'") or die(mysqli_error($conn));
								while($SMS1=mysqli_fetch_array($SSM1))
								{
								$invto_nm=$SMS1['nm'];
								}
...
<td align="left" valign="top"><a href="#" title="By : <?php  echo $eby;?> | On :<?php  echo $edt;?>"><b><?php  echo $sl;?></b></td>
<td align="left" valign="top"><b><?php  echo $dt;?></b></td>
<td align="left" valign="top"><b><?php  echo $sman;?></b></td>
<td align="left" valign="top"><b><?php echo $blnon;?></b></td>
<td align="left" valign="top"><b><?php  echo $cbill;?> <br> <font color="black">Ref / SF No : </font><?php  echo $sfno;?> 
<br> <font color="black">Invoice To : </font><?php  echo $invto_nm;?></b></td>
<td align="left" valign="top"><b><?php  echo $spn;?></b></td>
<td align="left" valign="top"><b><?php  echo $dldgr;?></b></td>
<td align="center" valign="top" align="right"><font color="red">Rs. <b><?php  echo $amm;?></b></font></td>
<td align="left" valign="top"><b><?php  echo $mtd;?></b><b></td>
<td align="left" valign="top"><b><?php  echo $mtddtl;?></b><b></td>
...
<?php  /*<a href="cash_report.php?sl=<?php  echo $sl1;?>" target="_blank" title="Click Here To Print Voucher"><i class="fa fa-print fa-2x" style="color:#333333;"></i></a>*/?>
</td>
<td align="center" valign="top">

<?php /*<a href="#" onclick="editt('<?php  echo $sl1; ?>')" title="Edit"><img src="images/edit.png" width="30"/></a>*/?>
...
?>							
<script>
$("#total").html('<font size="3"><b><?php echo number_format($total_am,2);?></b></font>');
</script>							