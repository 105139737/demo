...
$("#gbet").load("getbe.php?pcd="+prnm+"&brncd="+brncd+"&unit="+unit+"&betno="+betno+"&reffno="+reffno).fadeIn('fast');
	...
	function add1()
	{
		...
		{
		$('#wb_Text13').load('ser_adtmppr_sale.php?prnm='+prnm+'&unit='+unit+'&pcs='+pcs+'&prc='+prc+'&total='+total+'&disp='+disp+'&disa='+disa+'&lttl='+lttl+'&brncd='+brncd+'&fst='+fst+'&tst='+tst+'&cgst_rt='+cgst_rt+'&sgst_rt='+sgst_rt+'&igst_rt='+igst_rt+'&cgst_am='+cgst_am+'&sgst_am='+sgst_am+'&igst_am='+igst_am+'&refno='+encodeURIComponent(refno)+'&usl='+usl+'&bcd='+bcd+'&betno='+betno+'&tsl='+tsl+'&bill_typ='+bill_typ).fadeIn('fast');
		}
...
function temp()
	{
var bill_typ=document.getElementById('bill_typ').value;		
$('#wb_Text13').load("ser_tmppr_sale.php?bill_typ="+bill_typ).fadeIn('fast');
$('#fst_div').load("fst_get1.php").fadeIn('fast');
//$('#tst_div').load("tst_get1.php").fadeIn('fast');
}
function deltpr(un,sl)
{
$('#wb_Text13').load("ser_deltpr_sale.php?sl="+sl+"&tsl="+un).fadeIn('fast');
}
function t()
	{
var bill_typ=document.getElementById('bill_typ').value;	
...
<form method="post" target="" name="form1" id="form1"  action="ser_billing_gsts.php">
...
<tr>
	...
	<input type="text"  class="form-control" style="font-weight: bold;" id="no_servc" name="no_servc" tabindex="1" onkeypress="return isNumber(event)">
	</td>
	<td align="left" style="padding-top:15px;"><b>Month Duration</b>
	<input type="text" maxlength="4" class="form-control" style="font-weight: bold;" onkeypress="return isNumber(event)" id="dur_mnth" name="dur_mnth">
	</td>
	...
	<?php 
		...
		$spnm=$rwss['nm'];
		?>
		<option value="<?php  echo $spid;?>"><?php  echo $spnm;?> <?php if($rwss['mob']!=""){?>( <?php  echo $rwss['mob'];?> )<?php }?></option>
		<?php 
		}
	?>
	</select>
...
<td><b>Down payment:</b> 
<input type="text" class="form-control" id="dpay" name="dpay" onkeypress="return isNumber1(event)" style="text-align:right;">
</td>
<td><b>Finance Amount :</b> 
<input type="text" class="form-control" id="finam" name="finam" style="text-align:right;" onkeypress="return isNumber1(event)">
</td>
...
<td><b>EMI Month:</b> 
<input type="text" class="form-control" id="emi_mnth" name="emi_mnth" value="" onkeypress="return isNumber1(event)">
</td>
