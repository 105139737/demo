...
$disa_am=0;
$tpcs=0;
//select *,sum(pcs) as qty from  main_billdtls where sl>0 and cat='2' and FIND_IN_SET(blno, 'asd')>0 group by prsl,blno
//select * from main_billdtls where sl>0 and cat='2' and sync_stat=0 and betno is not null and betno!=''  $todts 

$data= mysqli_query($conn,"select main_billdtls.* from main_billdtls INNER JOIN main_billing ON main_billdtls.blno=main_billing.blno where main_billdtls.sl>0 and main_billdtls.cat='2' and main_billdtls.sync_stat=0 and main_billdtls.betno is not null and main_billdtls.betno!='' $bcd $todts $betno1 $blno1")or die(mysqli_error($conn));
