...
$datar= mysqli_query($conn,"SELECT * FROM main_call where stat='1' $all1 order by sl")or die(mysqli_error($conn));
$rcnt=mysqli_num_rows($datar);
$get=mysqli_query($conn,"select * from main_call where stat='1' $all1 order by sl limit $start,$ps") or die(mysqli_error($conn));
