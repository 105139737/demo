...
$exp_ledgr1="";
if($exp_ledgr!="" && $user_current_level>0)
{
  $exp_ledgr1=" and eby='$user_currently_loged'";
}
...
<th align="center">Amount <br>& Project</th>
...
$sl=$start;
$sln=0;
$datatt= mysqli_query($conn,"SELECT * FROM main_drcr where typ='44' and stat='1' and bill_typ='$btyp'".$brncd1.$pnoo.$qr.$ledg1.$exp_ledgr1)or die(mysqli_error($conn));
$rcntttl=mysqli_num_rows($datatt);
$datar= mysqli_query($conn,"SELECT * FROM main_drcr where typ='44' and stat='1' and bill_typ='$btyp'".$pnoo.$brncd1.$qr.$ledg1.$exp_ledgr1)or die(mysqli_error($conn));
$rcnt=mysqli_num_rows($datar);
$data= mysqli_query($conn,"select * from main_drcr where typ='44' and stat='1' and bill_typ='$btyp' $pnoo $brncd1  $qr $ledg1 $exp_ledgr1 order by dt Desc limit $start,$ps")or die(mysqli_error($conn));
...
$doc1=glob("img/expense/".$vno.".*");
if(count($doc1)>0)
{
$doc=explode("img/expense/".$vno,$doc1[0]);
$doc="img/expense/".$vno.$doc[1];
}
else
{
$doc="img/noimg.jpg";
...
<?php  } ?></td>
	<td align="center" valign="top">
<a href="#" onclick="sfdtl3expn('<?php  echo $sl1; ?>')" title="Edit"><img src="images/edit.png" width="30"/></a>
	</td>
	<td align="center" valign="top">
	<a href="expense_cash_report_new.php?sl=<?php  echo $sl1;?>" target="_blank" title="Click Here To Print Voucher"><i class="fa fa-print fa-2x" style="color:#333333;"></i></a>
	</td>
<td align="center" valign="top">
