...
$data1=mysqli_query($conn,"SELECT * FROM main_recv_app where sl>0 and blno='$blno_ref'")or die(mysqli_error($conn));
...
$query100 = "SELECT * FROM main_recv_dtl_app where  ref='$blno_ref' order by sl";
...
if($bill_no!="ADVANCE-PAYMENT")
{

$data= mysqli_query($conn,"SELECT sum(amm) as t1 FROM main_drcr where stat='1' and cbill='$bill_no' and brncd='$brncd' and cid='$cid' and dldgr='4'");
while ($row = mysqli_fetch_array($data))
{
$t1 = $row['t1'];
}
$data1= mysqli_query($conn,"SELECT sum(amm) as t2 FROM main_drcr where  stat='1' and cbill='$bill_no' and brncd='$brncd' and cid='$cid' and cldgr='4'");
while ($row1 = mysqli_fetch_array($data1))
{
$t2 = $row1['t2'];
}
