...
if($pnog==""){$pnog=1;}
$start=($pnog-1)*$ps;
$datattg= mysqli_query($conn,"SELECT sum(amm) as gtotal FROM main_drcr where typ='77' and stat='1'".$pnoo.$brncd1.$custid1.$todt)or die(mysqli_error($conn));
...
<input type="text" name="ps" id="ps" value="<?php  echo $ps;?>" size="7" onblur="pagnt1(this.value)">
       ...
       <font size="3"> <b> <?php echo number_format($gtotal,2);?> </b></font>
          </td>
...
$sln=0;
$total_am=0;
$datatt= mysqli_query($conn,"SELECT * FROM main_drcr where typ='77' and stat='1'".$pnoo.$brncd1.$custid1.$todt)or die(mysqli_error($conn));
$rcntttl=mysqli_num_rows($datatt);
$datar= mysqli_query($conn,"SELECT * FROM main_drcr where typ='77' and stat='1'".$pnoo.$brncd1.$custid1.$todt)or die(mysqli_error($conn));
$rcnt=mysqli_num_rows($datar);
 $data= mysqli_query($conn,"SELECT * FROM main_drcr where typ='77' and stat='1' $pnoo $brncd1 $custid1 $todt $slp1 order by dt Desc,sl desc limit $start,$ps")or die(mysqli_error($conn));
    ...
    <td align="left" valign="top"><b>Date :</b> <?php  echo $dt;?><br><b>JF No :</b> <?php echo $blnon;?><br><b>Bill No. :</b> <?php  echo $cbill;?></td>
