...
get_gstval();
 ...
 function gtt_unt()
 {
	prnm=document.getElementById('prnm').value;		
	 $("#g_unt").load("old_get_unt_pur.php?prnm="+prnm).fadeIn('fast');
 }
...
function add()
{
	...
	{
	 $('#wb_Text13').load("old_adtmppr1.php?prnm="+prnm+"&unit="+unit+"&usl="+usl+"&qnty="+qnty+"&mrp="+mrp+"&total="+total+"&disp="+disp+"&disa="+disa+"&ldis="+ldis+"&ldisa="+ldisa+'&lttl='+lttl+'&fst='+fst+'&tst='+tst+'&cgst_rt='+cgst_rt+'&sgst_rt='+sgst_rt+'&igst_rt='+igst_rt+'&cgst_am='+cgst_am+'&sgst_am='+sgst_am+'&igst_am='+igst_am+'&net_amm='+net_amm+'&bcd='+bcd+'&rate='+rate+'&betno='+betno+'&tsl='+tsl).fadeIn('fast');
	}
	}
	function reset()
	{
	...
	$('#wb_Text13').load("old_tmppr1_gst.php").fadeIn('fast');
	$('#fst_div').load("old_fst_get.php").fadeIn('fast');
	$('#tst_div').load("old_tst_get.php").fadeIn('fast');
	}
	
	function cal_main()
	{
	tdis=document.getElementById('tdis').value;
	$('#wb_Text13').load("old_cal_main.php?tdis="+tdis).fadeIn('fast');
	}
	function deltpr1(sl)
	{
	$('#wb_Text13').load("old_deltpr1.php?sl="+sl).fadeIn('fast');
	}
...
if(prnm=="Add")
{
	$('#cntt').load('admodel.php').fadeIn("fast");
	$('#modals').modal('show');
}
else
{
	$.get('old_get_gst_pur.php?dt='+dt+'&prnm='+prnm, function(data) 
	{
...
<body onload="tmppr1()" >
            ...
            <aside class="right-side strech">
                ...
                <section class="content-header">
                    <h1 align="center">Purchase(Old Stock) Return</h1>
                    <ol class="breadcrumb">
                        <li><a href="index.php"><i class="fa fa-home"></i> Home</a></li>
                        <li class="active">Purchase(Old Stock) Return</li>
                    </ol>
                </section>

                <!-- Main content -->
                <section class="content">
                   <form method="post" name="form1" id="form1" action="old_purchase_gsts.php">
...
?>
  ...
  <tr>
    <td align="right" style="padding-top:15px;dispaly:none;" width="15%"> <b>Original Invoice No. : </b></td>
	<td width="35%">
	<input type="text" class="form-control" tabindex="6"  id="inv"  name="inv" value="" size="35" placeholder="Enter Invoice No...">
	<input type="hidden" class="form-control" tabindex="6"  id="org_inv"  name="org_inv" value="" size="35" placeholder="Enter Invoice No...">
	</td>
    <td align="right" style="padding-top:15px;" width="10%"> <b>Original Invoice Date : </b></td>
	<td width="40%">
	<table width="100%">
	<tr>
	<td>
		<input type="text" class="form-control"  id="invdt" tabindex="7"  name="invdt" value="<?php  echo date('d-m-Y');?>" size="35" placeholder="Enter Date">
	
</td>
	
	<td align="right" style="padding-top:8px;"> <b> Return Date : </b></td>
	<td>
...
<td> 
<div id="prod_div">
<select id="prnm" name="prnm" tabindex="9"  onchange="get_gstval()" style="width:100%">
<option value="">---Select---</option>
...
<td>
<input type="text" id="betno" class="sc" tabindex="12"  autocomplete="off" style="text-align:center" name="betno">
</td>
<td>
...
<td align="center">
<input type="text" name="cgst_rt" id="cgst_rt" class="sc" tabindex="21"  class="sc" onblur="cal()" onfocus="this.select();"  style="text-align:center">
</td>
<td  align="center">
...
<td  align="center">
<input type="text" name="sgst_rt" id="sgst_rt" class="sc" tabindex="23"  class="sc" onblur="cal()" onfocus="this.select();"  style="text-align:center">
</td>
<td  align="center">
...
<td  align="center">
<input type="text" name="igst_rt" id="igst_rt" class="sc" tabindex="25"  class="sc" onblur="cal()" onfocus="this.select();"  style="text-align:center">
</td>
<td align="center">
...
<input type="hidden" name="tamm1" id="tamm1" tabindex="1"  class="form-control" style="background-color:#f3f4f5;" onkeypress="return isNumber1(event)" > 
 ...
 <td colspan="2">
 <b>Ledger :</b>
 <input type="hidden" class="form-control" id="remk" tabindex="1"  name="remk" value="" size="25" >
		<select  name="ledg" id="ledg" tabindex="1"  class="form-control">
		<?php  
		$gettt = mysqli_query($conn,"SELECT * FROM main_ledg order by sl") or die(mysqli_error($conn));
 ...
 <tr>
 <td>
<select class="sc" id="adl" name="adl" onchange="t2()" tabindex="1">
...
<table width="800px" class="table table-hover table-striped table-bordered" style="display:none;">
