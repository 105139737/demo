...
$mnth=$_REQUEST['mnth'];
$yr=$_REQUEST['yr'];
$filename ="GSTR_3B Report $yr-$mnth".".xls"; 
...
$igst_am=0; 
$log=0;
$data= mysqli_query($conn,"select *,sum(net_am) as net_am,sum(ttl) as amm,sum(cgst_am) as cgst_am,sum(sgst_am) as sgst_am,sum(igst_am) as igst_am from  main_billdtls where  blno='$invno' group by sgst_rt")or die(mysqli_error($conn));
...
$sgst_rt1=0;   
$sgst_am1=0; 
$data= mysqli_query($conn,"select *,sum(net_am) as net_am,sum(ttl) as amm,sum(cgst_am) as cgst_am,sum(sgst_am) as sgst_am,sum(igst_am) as igst_am from  main_purchasedet where blno='$blno' group by sgst_rt")or die(mysqli_error($conn));
while ($row = mysqli_fetch_array($data))
{
$amm1=$row['amm'];
$net_am1=$row['net_am'];
