...
<table>
  ...
  //  $data=mysqli_query($conn,"select * from main_product_prc where  sl>0 $cat1 $all1 group by psl order by edt,sl ASC LIMIT $offset, $no_of_records_per_page")or die(mysqli_error($conn));
  // $data=mysqli_query($conn,"SELECT *, MAX(sl) FROM main_product_prc WHERE sl>0 $cat1 $all1 GROUP BY psl LIMIT $offset, $no_of_records_per_page")or die(mysqli_error($conn));
  //echo mysqli_num_rows($data);

  $data=mysqli_query($conn,"SELECT t.sl,t.brand,t.cat,t.modelno,t.dis,t.disam,t.prc,t.psl,t.offprc,t.lprc
  FROM main_product_prc t
  INNER JOIN (
      SELECT MAX(sl) sl
      FROM main_product_prc
      GROUP BY psl
  ) b ON t.sl = b.sl WHERE t.sl>0 and t.status='0' $cat1 $all1 LIMIT $offset, $no_of_records_per_page")or die(mysqli_error($conn));
